...
HTML;
    ...
    protected function renderResetButton()
    {
        if (! empty($this->buttons['reset'])) {
            ...
            return "<button type=\"reset\" class=\"btn btn-warning float-start\"><i class=\"fas fa-rotate-left\"></i>&nbsp;{$reset}</button>";
        }
    }
    ...
    protected function renderSubmitButton()
    {
        if (! empty($this->buttons['submit'])) {
            return "<button type=\"submit\" class=\"btn btn-primary float-end\"><i class=\"fas fa-save\"></i>&nbsp;{$this->getSubmitButtonLabel()}</button>";
        }
    }
